...
- **A cancelled STREAMED Bedrock tool call records its blocked tool_result**: `chatStream()` broke out of the tool loop leaving a `tool_use` block with no matching `tool_result`, which is a malformed conversation the next Bedrock call rejects outright. `chat()` already did this. ([#282](https://github.com/ortus-boxlang/bx-ai/pull/282))
...
- **Cohere stream chunk-parse log qualified**: The error log for a malformed SSE frame read an unqualified `chunk.data` inside a `catch` body, which resolves against the class `variables` scope on BoxLang 1.13 and earlier. ([#281](https://github.com/ortus-boxlang/bx-ai/pull/281))
