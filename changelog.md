...
- **Mock provider propagates and announces stream-callback failures**: `MockService.sendStreamRequest()` emitted chunks by calling the caller's callback directly, so a callback that threw mid-stream escaped with no `onAIError` announce and skipped the `onAITokenCount` of a call the provider had already accounted for. It now emits through `emitStreamChunkGuarded()` with the shared error holder, stops the stream at the first failure like the real SSE transport, and re-raises when it owns the holder. ([#279](https://github.com/ortus-boxlang/bx-ai/pull/279))
...
- **Streaming callbacks**: every provider — `BaseService`/OpenAI-compatible, Claude, Cohere, Ollama and Gemini — caught an exception thrown by the *caller's own* stream callback inside its chunk-parse guard, logged it as a stream-parse error and carried the stream on, so a callback doing `chunk.choices.first()` on a usage-only final chunk failed on every stream with nobody the wiser. The callback now runs outside the parse guard, through the shared `BaseService::emitStreamChunkGuarded` seam, so the caller's error propagates out of `chatStream()` and is announced on `onAIError` exactly once — including from the post-stream emissions (Claude's tool-loop follow-up text and no-text fallback, Cohere's `emitStreamText` and resume terminator), which previously propagated with no announce at all. It is re-raised only *after* `onAITokenCount`, since the model was already called and billed by the time a chunk reached the caller and a billed call must not lose its usage accounting. Gemini's own normalizer keeps its own guard, so a frame with no `candidates` (a blocked prompt) is skipped and logged rather than aborting the stream and being blamed on the caller. Completes the same fix already made for Bedrock. ([#272](https://github.com/ortus-boxlang/bx-ai/issues/272))
